...
`GET /api/market/curves/:id`, `POST /api/market/bootstrap` (Quotes inkl. `FxSwapPoints`, `turnOfYear`, `globalSweeps`, `monotoneConvex`), `GET/PUT /api/market/snapshot` (`forwardJumps`), `PUT /api/market` (Spots, Fixings, Bewertungstag und – ohne kompletten Snapshot – `swaptionVols`/`capletVols`/`fxVols` je Key, Audit `market.vols`),
...
`Fixed`-Leg ohne `rate` oder `status: "Bogus"` ergibt 400 statt `pv: null`; Nominale und Beträge (`notional`, `buyAmount`/`sellAmount`, `payout`, Tilgungsstufen) sind auf 0 < x ≤ 10¹³ begrenzt (`1e300` → 400). Jede Route hat eine `operationId` und dokumentierte Antworten (2xx, 400/404/409/412/413/415/422/428/429/500);
