...
`POST /api/documents/termsheet|suitability|confirmation|kid` (`?format=md`), `GET/POST/PUT/DELETE /api/trades` (+ `/import` als JSON-Array oder CSV mit `content-type: text/csv` und `?type=` – elf Spaltenvorlagen für alle Builder-Produkte inkl. FX-Swap, Basis-, amortisierendem und IMM-Swap, Zeilen laufen durch die Core-Builder und das Trade-Schema, Fehler je Zeile –, `/from-template` für CrossCurrencySwap/FRA-Builder – CCS `collateralCurrency` default USD, sonst Quote-Währung des Paars, `null` = unbesichert; FRA-Index folgt der Periode, `3x6` → EURIBOR-3M),
`GET /api/market` (Übersicht inkl. `currencies`/`indices` – registrierte Währungen G5 + NOK/SEK/DKK/PLN und ihre Indizes), `GET /api/market/curves/:id`, `POST /api/market/bootstrap` (Quotes inkl. `FxSwapPoints`, `turnOfYear`, `globalSweeps`, `monotoneConvex`; auch in NOK/SEK/DKK/PLN mit `index: "NOWA"` usw.), `GET/PUT /api/market/snapshot` (`forwardJumps`; Import mit `warnings[]` `VOL_IMPLAUSIBLE:` für unplausible Vol-Flächen), `PUT /api/market` (Spots, Fixings, Bewertungstag und – ohne kompletten Snapshot – `swaptionVols`/`capletVols`/`fxVols` je Key, Audit `market.vols`; strukturell defekte Flächen → 400 `VOL_SURFACE_INVALID`, unplausible → 200 mit `warnings[]`),
...
**Vertrag (41 Operationen, OpenAPI 3.1.0):** Alle Bodies (inkl. Snapshot-Import) sind JSON-Schema-validiert; `POST /api/trades/import` deklariert `application/json` und `text/csv` als Request-Body; Trades sind eine diskriminierte Union über `type` mit typisierten Enum-Feldern – ein
...
Keine Fehler, sondern Präfixe in `warnings[]` einer 200-Antwort: `MISSING_FIXING:`, `MISSING_FX_FIXING:` (MtM-Reset oder Ausübungstag einer verfallenen FX-Option), `SETTLES_TODAY:`, `EXPIRED:`/`EXPIRES_TODAY:` (FX-Option nach bzw. am Verfall – abgewickelter Payoff, `analytics.lifecycle`, `greeksMethod: "settled-payoff"`), `COLLATERAL_CURVE_MISSING:`, `VOL_TYPE_CONVERTED:` (z. B. `model: "Black"` auf der Normal-Caplet-Fläche – PV ≈ Bachelier, `analytics.volConverted: "yes"`), `HAZARD_FLOORED:`.
...
Zeilen laufen durch die Core-Builder (Marktkonventionen der Währung) und werden einzeln gegen das `Trade`-Schema geprüft; jede Zeile erscheint im Ergebnis (`imported` | `skipped` | `rejected` mit `row`, `reason` und `code: CSV_ROW_INVALID` – auch bei Schemaverstößen wie einer `id` mit Leerzeichen; nur ein fehlender Pflicht-Header oder fehlendes `?type=` ist ein 400 `CSV_INVALID`), `?mode=upsert` ersetzt vorhandene IDs.
Gemeinsame optionale Spalten: `id`, `name`, `counterparty`, `book`, `uti`; `collateralCurrency` akzeptiert einen ISO-Code, leer (Builder-Default) oder `none`/`unbesichert`/`ohne` (unbesichert – wie die Spalte `collateral` der Web-Vorlage). Die vollständigen Vorlagen (Pflicht-/Optionalspalten, Beispielzeile) stehen in der OpenAPI-Beschreibung der Route und in `apps/api/src/lib/csv-import.ts`.
...
| `CrossCurrencySwap` | `pair domesticNotional effectiveDate tenor` (+ `fxSpot` **oder** `foreignNotional`) | `spread fixedRate domesticPayReceive frequency collateralCurrency` (`none` = unbesichert)                  |
