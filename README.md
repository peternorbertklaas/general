...
Core-Codes `INVALID_TRADE`, `NON_FINITE_PV`, `MISSING_RATE`, `MISSING_FIXING`, `NO_DISCOUNT_CURVE`, `CURVE_NOT_FOUND`, `NO_FX_SPOT`, `UNKNOWN_INDEX`, `UNKNOWN_CALENDAR`, `UNSUPPORTED_TRADE_TYPE`, `INVALID_FREQUENCY` (z. B. `7Q`), `UNKNOWN_DAYCOUNT`, `VOL_MODEL_INCOMPATIBLE` (Black auf nicht-positivem Forward/Strike ohne Shift), `INVALID_CREDIT_CURVE` (invertierte CDS-Quotes, `details.pillar`), `INVALID_TIMESTAMP` (400 beim Snapshot-Import, 422 im EMIR-Export), `INVALID_DATE`/`INVALID_TENOR` (400, aus `parseISO`/`parseTenor`, z. B. `2027-02-30`);
API-Codes `TOO_MANY_PERIODS` (400), `ID_MISMATCH`/`INVALID_QUERY_MAP`/`INVALID_REQUEST` (400), `NOT_FOUND` (404), `CONFLICT` (409), `PERIOD_BUDGET_EXCEEDED`/`STORE_BUDGET_EXCEEDED` (413), `CSV_INVALID`/`CSV_ROW_INVALID`, `SNAPSHOT_MALFORMED` (400), `SNAPSHOT_INVALID` (422), `PRECONDITION_FAILED` (412), `PRECONDITION_REQUIRED` (428), `RATE_LIMITED` (429), `INTERNAL_ERROR` (500), `DOMAIN_ERROR` – **jede** Fehlerantwort trägt einen dieser Codes.
Keine Fehler, sondern Präfixe in `warnings[]` einer 200-Antwort: `MISSING_FIXING:`, `MISSING_FX_FIXING:`, `SETTLES_TODAY:`, `COLLATERAL_CURVE_MISSING:`, `VOL_TYPE_CONVERTED:` (z. B. `model: "Black"` auf der Normal-Caplet-Fläche – PV ≈ Bachelier, `analytics.volConverted: "yes"`), `HAZARD_FLOORED:`.
...
**Rechenbudget:** Alle Bewertungen laufen synchron. Die API schätzt vor dem Rechnen die Kuponperioden: je Leg ≤ 1200 (`frequency: "1D"` über 100 Jahre → 400 `TOO_MANY_PERIODS` in < 50 ms, auch als `hedgingInstrument`),
...
**Betrieb hinter einem Proxy:** Das Rate-Limit (600/min) zählt je `request.ip`. Hinter einem Gateway ist das die Proxy-Adresse – `TRUST_PROXY=true` (jeder Hop) oder `TRUST_PROXY=<IP/CIDR-Liste>` lässt Fastify `X-Forwarded-For` auswerten, erst dann gilt das Limit je Client; `/api/health*` ist vom Limit ausgenommen. Request-Logs enthalten den Pfad ohne Query-String (siehe `SECURITY.md`).
