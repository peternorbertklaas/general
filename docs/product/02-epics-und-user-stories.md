...
- **US-8.7** ✅ Als Risk möchte ich eine Kurvenansicht mit Zero-/Forward-Chart, Vergleichskurve, editierbaren Quotes (Live-Re-Bootstrapping), Pillar-Tabelle und ±10bp-Buttons. **Runde 7 (R7-03):** „Kurve anlegen“ fokussiert den neuen Kurven-Tab, „Abbrechen“ den „+ Kurve“-Button.
...
- **US-8.9** ✅ Als Risk möchte ich eine Marktansicht: Bewertungstag, FX-Spots (editierbar), Swaption-ATM-Heatmap, FX-Vol-Fläche, Caplet-Vols, Kreditdaten. **Runde 7 (R7-01):** Fixings-Editor, FX-Fixings und CDS-Quotes folgen dem Roving-Tabindex (ein Tabstopp je Tabelle, `↑/↓`, `↵`/`F2`, `Esc`), Swaption-Cube, Caplet-Fläche und FX-Smile sind `role="grid"` mit einer aktiven Zelle (`←/→/↑/↓`, `↵`/`F2`, `Esc`) – statt 489–510 Tabstopps wenige Dutzend; dazu „+ Fläche“ (US-2.14) und „+ Paar“ (US-2.13). Tests `App.r7.test.tsx`, E2E „market tables and vol grids are one tab stop each“.
...
| Release | Inhalt |
|---|---|
| **v0.2 (dieses Repository)** | Alle ✅-Stories der Epics 1–8 und 10 (inkl. US-2.13 Kurven aus Quotes, US-6.6 Audit-Trail/Hashes, US-6.7 Dokumente, US-6.8 IFRS-13-Level, US-7.9/7.10 Validierung & API-Vertrag, US-8.15 Kundenmodus, US-8.16 Route-Splitting/Größenbudget, US-8.17 Chunk-Fehlerkarte), US-6.5 EMIR 🔶, Epic 9.1–9.5, 9.9 |
