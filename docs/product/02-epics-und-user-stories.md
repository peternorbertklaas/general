...
- **US-8.13** ✅ Als Berater möchte ich die App offline (ohne API) nutzen, weil der Bewertungskern im Browser läuft. Geliefert: Bewertung, Szenarien, Reports und Dokumente laufen vollständig im Browser, der Bestand liegt im lokalen Speicher; ein Service Worker (`apps/web/public/sw.js`, ohne Fremdbibliothek) cached die App-Shell – gebaute Assets cache-first, `index.html` network-first –, sodass nach dem ersten Online-Aufruf auch ein **Neustart/Reload ohne Netz** funktioniert (E2E-geprüft mit `context.setOffline(true)`); Topbar und Statusleiste zeigen „offline – lokaler Bestand“. Nicht enthalten: Marktdaten-Updates und die optionale API offline (Beispielmarkt bzw. importierter Snapshot bleiben nutzbar).
