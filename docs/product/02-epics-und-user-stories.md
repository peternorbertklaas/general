...
- **US-2.13** ✅ (Runde 6, Markt R6-5) Als Berater möchte ich eine Kurve für eine weitere Währung oder einen weiteren Index (NOK/SEK/DKK/PLN – NOWA, NIBOR, STIBOR, SWESTR, CIBOR, DESTR, WIBOR, POLONIA) ohne Codeänderung aus Quotes anlegen, um Termingeschäfte und Swaps in Nicht-G5-Währungen zu bewerten.
...
- **US-7.5** ✅ Als Entwickler möchte ich Trade-CRUD mit Validierung durch Probe-Bewertung sowie einen Batch-Import als JSON-Array oder CSV (`content-type: text/csv`, eine Spaltenvorlage je Produkttyp über `?type=`, deutsche Zahlen- und Datumsformate, Fehler je Zeile). Web-CSV (Runde 5): unmögliche oder unlesbare Daten in `start`, `maturity`, `deliveryDate`, `expiry` sind **Zeilenfehler** („Ungültiges Datum „31.02.2026“ in Spalte „start“ …“) statt stiller Standardwerte (R5-F1); die CCS-Vorlage trägt die CSA-Spalte `collateral` (Alias `csa`; leer = Standard, `none` = unbesichert) (Markt R5-3). **Runde 6 (Markt R6-2):** Vorlagen für **FX-Swap** (`FXS`), **Tenor-Basis-Swap** (`BASIS`), **amortisierenden Swap** (`AMORT`, Linear/Annuität, `finalNotional`) und **IMM-Swap** (`IMM`), Zinsstaffel-Spalte `stepUp` auf den Swap-Vorlagen, Barriere-Spalten `barrierType/barrierLevel/barrierRebate/barrierHit` auf der FX-Option – Spaltennamen identisch mit der API-CSV; Zeilen, die die Trade-Validierung nicht bestehen, stehen als Zeilenfehler im Import-Dialog (R6-06).
...
- **US-8.2** ✅ Als Berater möchte ich Bloomberg-artige Schnelleingabe in natürlicher Kurzform (`irs 10y pay 3.1% 10m`, `collar 7y 3.5/1.5 6m`, `swpt 1y5y payer 3% 10m`, `fxf eurusd -2m 1.1725 15.03.2027`, `fxo eurusd put 1.15 3m 9m`) mit Live-Vorschau und Enter-Anlage.
