...
|---|---|---|---|
| **Blotter** | `g b` / `Alt+1` | Portfolio-Überblick | KPI-Kacheln, sortierbare Spaltenköpfe (Buttons mit `aria-sort`), Spaltenauswahl inkl. **Buch** (lokal gespeichert), Filter-Chips (Typ, „Indikationen ausblenden“, „ohne Kontrahent“, **„ohne UTI“** = EMIR-Meldelücke), **Gruppieren nach** Kontrahent / Buch / Typ mit Zwischensummen PV und DV01, Suche (ID, Name, Kontrahent, Buch, UTI), Sortierung/Filter/Gruppierung persistiert, Status-Badge mit **„abgelaufen“** für Angebote (`Quoted`) hinter dem Gültigkeitsdatum, Leerzustände mit Handlungsaufforderung, Kontextmenü (Rechtsklick, Roving-Fokus), Hinweis-Popover je Trade, Menü **„⤓ Export ▾“**: CSV (sichtbare Spalten) / JSON / EMIR (UTI, Clearing-Felder und Transaktionspreis → MTMA aus den Report-Eingaben) / **Portfolio-Report** (JSON + Markdown: PV, DV01, Theta, FX-Delta je Kontrahent, Buch, Typ mit Snapshot-ID, Inputs- und Report-Hash – Kern `buildPortfolioReport`), **Import JSON und CSV** (Spaltenvorlagen IRS / FXF / CAP zum Download, deutsche oder englische Kopfzeilen, Dezimalkomma, Datum ISO/DE/Tenor), bei ID-Kollision Dialog **überspringen / ersetzen / umbenennen**; Trades mit Fehler-Validierung zeigen „Fehler“ und werden nicht bewertet, nicht summiert und nicht exportiert; Onboarding-Hinweis; Beispielbuch enthält `CCS-0001` und `FRA-0001` |
...
| **Kurven** | `g c` / `Alt+3` | Kurvenaufbau | Zero + 6M-Forward, Vergleich (gleiche Währung), **Interpolation je Kurve wählbar** (log-linear, linear Zero, kubischer Spline, flat forward, **monoton-konvex** nach Hagan–West) – Overrides liegen im Store, überleben den Stichtagswechsel, bootstrappen abhängige Kurven neu und zählen als „Markt modifiziert“; **Turn-of-Year** (Datum + bp, „Anwenden“) als Forward-Sprung auf der gewählten Kurve (`turnOfYear` der Bootstrap-Spec, persistiert, Badge an der Kurve); Kurven-Tabs inkl. **JPY-TONA**; Quotes im Store (bleiben beim Ansichts- und Stichtagswechsel), geänderte Quotes orange mit Original im Tooltip, FX-Swap-Punkte-Quotes editierbar in Punkten (`quoteLabel`/`bumpQuote` des Kerns), `Ctrl+Z` macht Quote-Änderungen rückgängig, Pillar-Datum und Residuum je Quote, `EUR-ESTR-USDCSA` (Xccy-Basis) |
...
| **Markt** | `g m` / `Alt+5` | Snapshot | Bewertungstag-Popover, Spots (Teil des Quote-Sets), Fixings-Editor (Tenor-Datumsfeld), Swaption-Heatmap (ARIA-Tabelle), FX-Smile, Caplet-Vols, **Kreditdaten mit CDS-Termstruktur** je Kontrahent (Tenor/Spread-Zeilen, Recovery → `bootstrapHazardCurve`, Pillars mit Hazard und Überlebenswahrscheinlichkeit; der Report übergibt sie als `cptyHazardCurve` an das XVA), flache Hazard-Raten des Snapshots als Referenz, Snapshot-Export/-Import, „Markt zurücksetzen“ (Quotes + Interpolation) |
| **Report** | `g r` / `Alt+6` | Prüfungsfähig | Expliztes „Report erzeugen“ (fester Zeitstempel), Engine-Version, Snapshot-ID (mit „Quotes modifiziert“ im Label, JSON und Druckkopf), Report-Hash, **Governance-Zeile** (Snapshot-Status indikativ/freigegeben, Datenquellen, Modellversion, Validierer), **Perspektive Bank / Kunde** (Standard Kunde; Kundenmodus erzwingt Kunde) mit Vorzeichenregel, **What-if-Kennzeichnung** („nicht prüfungsfähig“, auch im JSON), Kostentransparenz – Eingaben je Trade im Store persistiert, „Standardwerte“-Button, „Eingaben geändert“ auch nach Quote-Änderung (stabiler Quote-Hash) –, Exposure (CVA mit CDS-Termstruktur, wenn im Markt hinterlegt), Methodik (deutsch), Druck-Layout (A4, Kopfzeile), Dokumente **Termsheet** (mit anfänglichem Marktwert, Dezimalkomma), **Geeignetheitserklärung**, **Confirmation** (Parteien mit LEI, DRV / ISDA, Datum, Referenz – live aus den Eingaben) und **Basisinformationsblatt (KID)** (Hersteller, empfohlene Haltedauer, Standard-Szenarien) – druckbar ohne Leerseite, Titel dunkel auf hell; im Kundenmodus sind alle Dokumente erlaubt, interne Margenzeilen entfallen bis auf die gesetzlich geforderten |
...
| | `n z` `n r` | Neu: Cross-Currency-Swap (EUR/USD 5Y, €STR −20 bp vs SOFR), FRA (EUR 3x6) |
...
| | `Ctrl/⌘+Shift+L` | Portfolio-Report (JSON) herunterladen – Markdown über das Export-Menü |
...
| | `Ctrl/⌘+Shift+K` | Basisinformationsblatt (PRIIPs-KID) öffnen |
| | `Ctrl/⌘+Shift+F` | Confirmation (Geschäftsbestätigung unter DRV / ISDA) öffnen |
...
ccs|xccy      <pair> <tenor> [spreadbp] [notional] [mtm] [pay|rec] [fxSpot]  (Basis-Spread auf dem ersten Leg; Fremdnominal aus Markt-Spot oder angegebenem Kurs; mtm = MtM-Reset)
fra           [ccy] <NxM> pay|rec <rate%> [notional] [index]                  (Periode ab Spot-Datum, Index = EURIBOR der Periodenlänge)
