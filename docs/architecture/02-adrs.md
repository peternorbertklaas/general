...
**Entscheidung.** CI (`.github/workflows/ci.yml`, Push auf `main`/`claude/**` und Pull Requests, Node 20 und 22) führt in dieser Reihenfolge aus: Install mit eingefrorenem Lockfile → Core-Build → Typecheck → ESLint (`--max-warnings 0`) → **Prettier-Check** (`pnpm format:check`, `.prettierignore` für Build-Artefakte und Prosa-Doku) → Tests mit Coverage-Schwellen in allen drei Paketen (Coverage-Artefakte für Core, API, Web) → **`pnpm audit --prod --audit-level=high`** → API- und Web-Build → Playwright-E2E-Smoke gegen den Produktions-Build (Node 22). Das Web-Artefakt enthält keine Source-Maps. Dependabot hält npm- und Actions-Abhängigkeiten wöchentlich aktuell; CODEOWNERS erzwingt Reviews auf Core, API, Compliance-Doku und CI.
**Konsequenzen.** + Formatierung, Abhängigkeitsrisiken und Coverage sind Gates, nicht Empfehlungen. − Ein neues High-Advisory ohne Fix blockiert den Merge, bis ein Override (`pnpm.overrides`) oder eine dokumentierte Ausnahme vorliegt.
