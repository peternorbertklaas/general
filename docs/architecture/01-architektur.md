...
| `apps/web` | React 19, Vite 8 (Rolldown), zustand, ECharts, CSS-Tokens | Workstation-UI, Hotkey-System, Command Palette, Views |
| CI | GitHub Actions (Node 22/24; Node ≥ 22.22 laut `engines`, `.nvmrc`, `engine-strict`) | Job `commitlint` (Conventional Commits, Betreff ≤ 72 Zeichen, Scopes aus `commitlint.config.mjs` – nur die Commits des Pushs bzw. der PR-Range, kein lokaler Hook) parallel zu `build-test`: Install (frozen lockfile) → `version.ts`-Drift-Check → Core-Build → Typecheck → ESLint (typbewusst für Core und API, `recommendedTypeChecked`) → Prettier-Check → Tests mit Coverage-Schwellen (Core/API/Web) → `pnpm audit --prod` (ein Retry) → Builds (ohne Source-Maps im Artefakt) → Playwright-E2E (ADR-020); Push auf `main`/`claude/**`, PRs gegen `main` ohne Doppel-Lauf; Dependabot, CODEOWNERS |
...
dates/       date (SerialDate, Tenor, IMM), calendar (TARGET/US/US-SIFMA/UK/CH/JP/DE/NO/SE/DK/PL – gegen QuantLib
             abgeglichen –, Joint, Custom: `CustomCalendar`/`CustomCalendarJson`, `registerCalendar`, `isBuiltInCalendar`,
             `listCustomCalendars`; Feiertags-Feed-Overlay `registerCalendarHolidays`; BDC, advance),
...
curves/      Curve-Interface, InterpolatedCurve (5 Interpolationen, Flat-Forward-/Flat-Zero-Extrapolation, Shifts, Roll),
             index-definitions (RateIndex mit optionalem `paymentCalendar`, SwapConventions; Register `registerRateIndex`/
             `registerSwapConventions` mit den Validatoren `validateRateIndex`/`validateSwapConventions`, `isBuiltInIndex`,
             `knownCurrencies`/`knownIndices`), bootstrap (sequentiell, dual-curve, Brent je Pillar,
...
- Ressourcen (44 Operationen, jede mit `operationId`): `/api/market` (Übersicht inkl. JPY-TONA, `source` sample | import, registrierte `currencies`/`indices`/`conventions`/`calendars`, Kurven, Vols, Bootstrap mit Futures/Basis/XCCY/FX-Swap-Points, Pillar-Merge, `turnOfYear`, `globalSweeps`, Interpolation inkl. `monotoneConvex`; `POST /api/market/curves` speichert die Kurve, merkt sich Spezifikation und Quotes (`parRiskTracked`, R8-3/N8-01) und setzt für eine Währung ohne Diskontkurve die Zuordnung (`isDiscountCurve`, R7-3); `POST /api/market/indices`/`/conventions`/`/calendars` registrieren weitere Indizes, Währungen und Feiertagskalender im Core-Register (eingebaute Indizes und Kalender nicht ersetzbar, Audit `register.*`, ADR-027); `PUT /api/market` setzt Spots, Fixings, `discountCurveId`/`collateralDiscountCurveId` (Kurve existiert und notiert in der Währung des Schlüssels – N8-02) und ersetzt Vol-Flächen je Key – `swaptionVols`/`capletVols`/`fxVols` mit den Snapshot-Schemas, Audit `market.vols`, R4-5 –; der Stichtagswechsel `valuationDate` baut den Sample-Markt neu und nimmt Laufzeitkurven (Re-Bootstrap aus den gemerkten Quotes), Zuordnungen, Vol-Overrides, Fixings, Spots und Credit mit, rollt einen importierten Snapshot (`rollMarket`; `meta.snapshotTime` entfällt, das Label wird `(rolled to <Datum>)` markiert – EMIR-Feld 23 folgt dem neuen Stichtag, N9-02) oder verwirft ihn mit `discardImport: true`; nachgebootstrappte Laufzeitkurven erhalten die Diskontkurven-Regel erneut (N9-03); Verworfenes steht in `warnings[]` als `MARKET_STATE_DROPPED:` (N8-01)), `/api/market/snapshot` (Export/Import mit `forwardJumps`; API-Envelope `calendars`/`indices`/`conventions` für registrierte Einträge und `quotes` – `{ curveId, spec: CurveBuildSpec }` je Laufzeitkurve mit gemerkten Quotes, Import legt sie als Laufzeitkurven-Quotes ab, R9-1 –, ADR-027 – Import atomar nach Prüfung des ganzen Envelopes mit den Core-Validatoren (N8-04; zusammengesetzte Kalender-Ids `CZ+TARGET` je Komponente ersetzt, N9-01), Export-ETag `"<snapshotId>"` bzw. `"<snapshotId>-<registerHash>"` über Register und Quotes (N8-03/R9-1), `X-Market-Snapshot-Id` bleibt die Markt-ID), `/api/price`, `/api/price/portfolio`, `/api/risk`, `/api/risk/par`, `/api/risk/par/portfolio` (Specs aller Kurven mit Quotes im Store – Sample, Laufzeitkurven und importierte Kurven mit `quotes`-Eintrag –, `curvesWithoutQuotes[]` und `PAR_RISK_INCOMPLETE:` für den Rest, R8-3/R9-1), `/api/risk/vega` (`dimension` expiry | expiry-tenor, `smile` für RR25/BF25-Buckets der FX-Vol-Fläche; `kind` swaption | caplet | fx), `/api/scenarios` (`includeHistorical`), `/api/scenarios/standard|historical`, `/api/scenarios/grid`, `/api/xva` (Hazard-Termstruktur), `/api/xva/hazard-curve` (`floorHazard`, `warnings`), `/api/hedge/effectiveness` (`designationSnapshot`, `freezeDesignationVol`), `/api/hedge/hypothetical` (`designation`, Tilgungspläne), `/api/report` (JSON | `?format=csv`; `perspective` mit dokumentierter Vorzeichenregel, `governance`), `/api/report/portfolio` (Buchebene: PV/DV01/Theta/FX-Delta je Trade, Aggregate nach Kontrahent/Buch/Produktart, `groupBy`; JSON | `?format=md`), `/api/documents/termsheet|suitability|confirmation|kid` (JSON | `?format=md`), `/api/emir/valuations` (`GET` mit `?asOf=&timestamp=&method=&uti=&transactionPrice=` für kleine Bücher – Query-Maps ≤ 4 kB – und `POST` mit denselben Optionen und den Maps als JSON-Objekte; Clearing-Felder mit den Wertebereichen nach ITS 2022/1860 Tabelle 2 aus dem Core, `intentToClear` für Feld 31 `I`), `/api/trades` (CRUD, `/import` als JSON-Array oder CSV – `content-type: text/csv` als zweiter Request-Body-Medientyp im Vertrag, `?type=` wählt eine der elf Spaltenvorlagen (jede Vorlagendatei beginnt mit der Spalte `type` und dem Typ-Token der Workstation, der Reader gleicht sie mit `?type=` ab – R9-4), `?dryRun=1` prüft und bewertet ohne zu speichern, unbekannte Query-Parameter → 400, Zeilen werden über die Core-Builder `makeVanillaSwap`/`makeAmortisingSwap`/`makeImmSwap`/`makeBasisSwap`/`makeFxForward`/`makeFxSwap`/`makeCapFloor`/`makeSwaption`/`makeFxOption`/`makeCrossCurrencySwap`/`makeFra` gebaut und einzeln gegen das `Trade`-Schema geprüft, abgelehnte Zeilen mit Zeilennummer und `CSV_ROW_INVALID` im Ergebnis (R6-2/R6-3) –, `/from-template` – diskriminiert über `template` für die Core-Builder `makeCrossCurrencySwap` (`collateralCurrency` default USD bzw. Quote-Währung, `null` = unbesichert) / `makeFra` (Index je Periode, `3x6` → EURIBOR-3M)), `/api/audit`, `/api/health`, `/api/health/ready`, `/docs/json` (OpenAPI immer; Swagger UI `/docs` nur außerhalb `production`).
...
- Fehlercodes: `ErrorResponse.code` trägt die vollständige Liste als `examples` (ergänzt durch `openApiTransform`, Quelle `API_ERROR_CODES` in `schemas.ts`; 45 Codes – 24 Core, 21 API) und beschreibt jeden Code – Core-`PricingErrorCode` (u. a. `INVALID_FREQUENCY`, `UNKNOWN_DAYCOUNT`, `TOO_MANY_PERIODS`, `VOL_MODEL_INCOMPATIBLE`, `INVALID_CREDIT_CURVE`, `INVALID_TIMESTAMP`, `INVALID_CALENDAR` (400 auf `POST /api/market/calendars`), `INVALID_DATE`/`INVALID_TENOR` – die beiden letzten als einzige `PricingError`-Codes mit 400 bei der Bewertung) und API-Codes (`CSV_INVALID`, `SNAPSHOT_MALFORMED`, `SNAPSHOT_INVALID`, `PERIOD_BUDGET_EXCEEDED`, `STORE_BUDGET_EXCEEDED`, `PRECONDITION_FAILED`/`_REQUIRED`, `ID_MISMATCH`, `INVALID_QUERY_MAP`, `INVALID_REQUEST`, `NOT_FOUND`, `CONFLICT`, `RATE_LIMITED`, `INTERNAL_ERROR`); Inline-Fehler der Routen laufen ausnahmslos über `sendError` (`lib/errors.ts`; auch 412/428 der Preconditions und die 400er des Snapshot-Imports, Core-Codes über `apiErrorCode` gegen den Katalog geprüft – Grep-Test in `review-r6.test.ts`), sodass jede Fehlerantwort – auch 404/409 und der Not-Found-Handler – einen Katalog-Code trägt (N4-05/N6-03); Medientypen außer `application/json` (und `text/csv` auf der Import-Route) antworten 415 `UNSUPPORTED_MEDIA_TYPE` – der eingebaute `text/plain`-Parser ist entfernt; die zwölf Warnungs-Präfixe aus `WARNING_PREFIXES` (`MISSING_FIXING:`, `MISSING_FX_FIXING:`, `SETTLES_TODAY:`, `COLLATERAL_CURVE_MISSING:`, `VOL_TYPE_CONVERTED:`, `HAZARD_FLOORED:`, `EXPIRED:`, `EXPIRES_TODAY:`, `BARRIER_STATE_UNKNOWN:`, `VOL_IMPLAUSIBLE:`, `MARKET_STATE_DROPPED:`, `PAR_RISK_INCOMPLETE:`) sind keine Fehler, sondern Einträge in `warnings[]` einer 200-Antwort.
- Datumsformat: ISO-8601-Strings an der API-Grenze, intern Serial-Dates (ADR-007, Test `lib/dates.test.ts`).
- Validierung: JSON-Schema je Body/Query/Params (ADR-013) – **alle** Bodies inkl. Snapshot-Import; Trades als diskriminierte Union (`oneOf` je `type`, typisierte Enum-Felder, `additionalProperties: false`); semantische Prüfung durch den Core (`PricingError` → 422 mit `code`); Fehlerformat `{ error, code, details?, statusCode, validation?, requestId }` auf allen Routen – jede Fehlerantwort inkl. Schema-Verstößen (`VALIDATION_ERROR`) und JSON-Parse-Fehlern (`INVALID_JSON`) trägt einen Katalog-Code (ADR-025); Vol-Flächen strukturell geprüft → 400 `VOL_SURFACE_INVALID` (Markt R5-1); Nominale/Beträge ≤ 10¹³.
- OpenAPI-Vertrag (**3.1.0**): `servers`, `operationId` je Route, Response-Schemas (2xx + 400/404/409/412/413/415/422/428/429/500; 415 nur auf Operationen mit Request-Body – `responses(...)` vs. `responsesWithoutBody(...)` in `schemas.ts`, N6-03), benannte `components.schemas` aus den `$id`s der geteilten Schemas (`Trade`, `InterestRateSwap`, `FRA`, `CapFloor`, `Swaption`, `FxForward`, `FxSwap`, `FxOption`, `CrossCurrencySwap`, `SwapLeg`, `FixedLeg`, `FloatLeg`, `FromTemplateCrossCurrencySwap`, `FromTemplateFra`, `MarketSnapshot`, `CurveBuildSpec` (Bootstrap-Spezifikation – Body `spec` von `POST /api/market/bootstrap|curves` und `spec` der `quotes`-Einträge des Snapshots), `RateIndex`, `SwapConventions`, `CustomCalendar`, `ErrorResponse` – 20 Komponenten), `discriminator.mapping` auf jeder Union (Ajv validiert über `$ref`-Zweige, `openApiTransform` in `app.ts` ergänzt das Mapping im Dokument); Vertragstest `apps/api/src/contract.test.ts` (Operationsliste) und `review-r3.test.ts` (Namen, alle `$ref` auflösbar, Mapping) (ADR-023).
- Semantik: `POST /api/trades` 201/409 (`?upsert=1`), `PUT` mit `If-Match`/starkem ETag → 412 bei Abweichung (starker Vergleich nach RFC 9110, `W/`-Tags matchen nie) und Body-`id` ≠ Pfad-`id` → 400, `GET` mit `If-None-Match` → 304 (schwacher Vergleich), `DELETE` 204 (`If-Match` → 412 bei Abweichung); `If-Match` ist standardmäßig optional, mit `REQUIRE_IF_MATCH=1` Pflicht (428 `PRECONDITION_REQUIRED` mit `currentEtag`); Trade-IDs `^[A-Za-z0-9._-]{1,64}$`; Probe-Bewertung in der Reporting-Währung aus `?reportingCurrency=` (Default EUR).
- Header: `X-Request-Id` (eingehende `x-request-id` wird übernommen), `X-Market-Snapshot-Id` auf jeder bewertungsbezogenen Antwort (= `audit.snapshotId` des Reports, F-05), starker `ETag` (`"version-hash"` bzw. `"snapshotId"`) auf Trades und Snapshot.
- Sicherheit: helmet, Rate-Limit je Quell-IP inkl. unbekannter Routen (`TRUST_PROXY` für `X-Forwarded-For` hinter einem Gateway, Health-Routen ausgenommen), CORS-Allowlist, Body-Limit, sichere Dateinamen, Request-ID (ADR-018); Audit-Trail mit Hash-Kette (ADR-014); Logger mit `LOG_LEVEL`/Redaction und Request-Logs ohne Query-String (ADR-025); Abhängigkeits-Audit und Dependabot (ADR-020).
- Marktquotes und Nutzerzustand: `MarketRepository` hält die aktuellen Quotes je Sample-Kurve **und** die Bootstrap-Bodies aller Laufzeitkurven (`rememberCurve`), die per `PUT /api/market` gesetzten Vol-Overrides (`rememberVols`) und die Herkunft des Markts (`source`); `POST /api/market/curves` ersetzt Kurve und Quotes, der Snapshot-Import legt die `quotes` des Envelopes über `rememberQuotes` in denselben Slot (R9-1; `listQuotes()` liefert sie für Export und ETag zurück), `parRiskSpecs()` liefert die Specs aller Kurven mit Quotes an `/api/risk/par`, und `rebuild(valuationDate, { discardImport })` baut den Sample-Markt neu, bootstrappt die Laufzeitkurven in Ladereihenfolge nach, trägt Diskont-/Collateral-Zuordnungen und Vol-Overrides weiter (gemeinsame Auflösung der Bodies in `lib/curve-specs.ts`), wendet auf jede nachgebootstrappte Kurve die Diskontkurven-Regel von `POST /api/market/curves` erneut an (`isDiscountCurve: false` nie, sonst „erste Kurve einer Währung ohne Diskontkurve“ – N9-03; die `MARKET_STATE_DROPPED:`-Warnung einer verlorenen Zuordnung nennt den Ersatz) oder rollt einen importierten Snapshot (`rollMarket`; `rolledMeta` entfernt den veralteten `meta.snapshotTime` und markiert das Label `(rolled to <Datum>)` – dieselbe Regel wie im Core seit R9, N9-02) – wie `rebuildMarket` der Workstation (R7-F1/R8-F2). `RegisterStore` merkt sich je API-Instanz die registrierten Kalender/Indizes/Konventionen für Envelope und ETag (`hash(quotes)` – FNV-1a über Register **und** die `quotes` des Marktstores); der Envelope wird vor dem Registrieren als Ganzes mit den Core-Validatoren geprüft (`lib/register-validation.ts`, N8-04 – anstehende Kalender werden auch als Komponente einer zusammengesetzten Id `CZ+TARGET` ersetzt, `substitutePendingCalendar`, N9-01), ebenso die Währung der Collateral-Zuordnungen (N8-02) und die `quotes`-Einträge (`quotesProblems`: Kurve im Snapshot, `spec.id`/`spec.currency` passend, Index registriert oder im Envelope).
