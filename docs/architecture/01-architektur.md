...
- Marktquotes und Nutzerzustand: `MarketRepository` hält die aktuellen Quotes je Sample-Kurve **und** die Bootstrap-Bodies aller Laufzeitkurven (`rememberCurve`), die per `PUT /api/market` gesetzten Vol-Overrides (`rememberVols`) und die Herkunft des Markts (`source`); `POST /api/market/curves` ersetzt Kurve und Quotes, `parRiskSpecs()` liefert die Specs aller Kurven mit Quotes an `/api/risk/par`, und `rebuild(valuationDate, { discardImport })` baut den Sample-Markt neu, bootstrappt die Laufzeitkurven in Ladereihenfolge nach, trägt Diskont-/Collateral-Zuordnungen und Vol-Overrides weiter (gemeinsame Auflösung der Bodies in `lib/curve-specs.ts`) oder rollt einen importierten Snapshot (`rollMarket`) – wie `rebuildMarket` der Workstation (R7-F1/R8-F2). `RegisterStore` merkt sich je API-Instanz die registrierten Kalender/Indizes/Konventionen für Envelope und ETag (`hash()`); der Envelope wird vor dem Registrieren als Ganzes mit den Core-Validatoren geprüft (`lib/register-validation.ts`, N8-04), ebenso die Währung der Collateral-Zuordnungen (N8-02).
