...
- Ressourcen (39 Operationen, jede mit `operationId`): `/api/market` (Übersicht inkl. JPY-TONA, Kurven, Vols, Bootstrap mit Futures/Basis/XCCY/FX-Swap-Points, Pillar-Merge, `turnOfYear`, `globalSweeps`, Interpolation inkl. `monotoneConvex`), `/api/market/snapshot` (Export/Import mit `forwardJumps`, ETag), `/api/price`, `/api/price/portfolio`, `/api/risk`, `/api/risk/par`, `/api/risk/par/portfolio`, `/api/risk/vega` (`dimension` expiry | expiry-tenor), `/api/scenarios` (`includeHistorical`), `/api/scenarios/standard|historical`, `/api/scenarios/grid`, `/api/xva` (Hazard-Termstruktur), `/api/xva/hazard-curve`, `/api/hedge/effectiveness`, `/api/hedge/hypothetical` (`designation`, Tilgungspläne), `/api/report` (JSON | `?format=csv`; `perspective` mit dokumentierter Vorzeichenregel, `governance`), `/api/documents/termsheet|suitability|confirmation|kid` (JSON | `?format=md`), `/api/emir/valuations` (`?asOf=&timestamp=&method=&uti=&transactionPrice=`, Clearing-Felder), `/api/trades` (CRUD, `/import`, `/from-template` – diskriminiert über `template` für die Core-Builder `makeCrossCurrencySwap`/`makeFra`), `/api/audit`, `/api/health`, `/api/health/ready`, `/docs/json` (OpenAPI immer; Swagger UI `/docs` nur außerhalb `production`).
