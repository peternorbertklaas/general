...
- **IFRS 13 / IDW RS HFA 47 – Bewertungsrahmen im Report [E]:** Methodikabschnitt und Feld `governance` (Snapshot-Status indikativ/freigegeben, Marktdatenquellen, Modellversion, Validierer) dokumentieren Bewertungstechnik (Income Approach), Input-Hierarchie und Modellfreigabe (MaRisk AT 4.3.5, BTO 2.2.1); IDW RS HFA 35 gilt ausschließlich für Bewertungseinheiten nach § 254 HGB (Hedge-Accounting-Modul, siehe oben). Die Level-Heuristik prüft Extrapolation nur gegen die vom Geschäft tatsächlich genutzten Kurven (Diskontkurve unter dem CSA des Geschäfts, Projektionskurven der referenzierten Indizes) sowie Optionslaufzeit/Tenor gegen das Gitter der genutzten Volatilitätsfläche.
...
- **Swap-Konventionen je Währung [E, Interdealer-Standard nach Bloomberg SWPM / LSEG; Umsetzung `packages/pricing-core/src/curves/index-definitions.ts`, `SWAP_CONVENTIONS`/`RATE_INDICES`, seit Runde 6 zur Laufzeit erweiterbar über `registerRateIndex`/`registerSwapConventions` (Markt R6-5):**
  ...
  Die NOK/SEK/DKK/PLN-Zeilen folgen den veröffentlichten Indexkonventionen (Norske Finansielle Referanser, Swedish Financial Benchmark Facility, Danish Financial Benchmark Facility, GPW Benchmark) und sind indikativ, wo ein Markt mehrere Varianten quotiert (SEK: STIBOR-3M ist der liquide Tenor, DKK: CIBOR-6M); ein Desk mit abweichenden Terms überschreibt sie per `registerSwapConventions`. Kurven in diesen Währungen werden über `POST /api/market/bootstrap` bzw. `/api/market/curves` mit den registrierten Indizes gebootstrappt (z. B. `index: "NOWA"`); der Beispielmarkt selbst bleibt G5 (US-2.9). FX-Vol-Flächen für Kreuze mit diesen Währungen sind im Beispielmarkt nicht enthalten – die Workstation legt sie per „+ Fläche“ an (US-2.14, seit Runde 7 als strukturelle Ergänzung, die Import und Reload überlebt), die API per `PUT /api/market { fxVols }`; bis dahin bewertet der Kern mit dem 8-%-Fallback (Level 3) und die Palette nennt den Weg. Weitere Währungen (CZK, HUF, …) werden zur Laufzeit registriert – Kalender (`POST /api/market/calendars`, Core `registerCalendar`), Indizes und Konventionen (Runde 7/8, ADR-027) – und stehen über den Snapshot-Envelope auch der Workstation zur Verfügung.
...
**Umsetzung im Core [E]:** Jede `InterpolatedCurve` trägt ihre Herkunft in `meta.source` (`"bootstrap"` aus `bootstrapCurve`, `"import"` aus einem Snapshot/Vendor-File, `"flat"` aus `flatCurve`); der Methodiktext des Bewertungsreports beschreibt die Kurvenkonstruktion daraus – „sequentielles Bootstrapping (Brent je Pillar)“ nur für tatsächlich gebootstrappte Kurven, sonst „importierte Kurve (Pillars aus Snapshot)“. **Turn-of-Year:** der Sprung des instantanen Forwards wird als Kalenderereignis geführt – Default-Fenster ist die Geschäftstagsperiode über den Jahreswechsel auf dem Indexkalender (letzter Geschäftstag des Jahres bis erster Geschäftstag des Folgejahres, 2026/27: Do 31.12. → Mo 04.01. = 4 Kalendertage), so dass der DF-Effekt der Marktquotierung (Aufschlag auf den ON-Satz über die Jahresendperiode) entspricht; beim Constant-Curve-Roll (Theta, Zeitszenarien) bleibt der Sprung auf seinem Kalenderdatum und verschiebt sich nicht mit der Kurve.

**Interpolation [Q/E]:** Marktstandard ist log-linear auf Diskontfaktoren (= stückweise konstante Forward-Rates; stabil, aber „sägezahn"-Forwards) oder **Monotone Convex (Hagan/West 2006)** für glatte, arbitragefreie Forwards. Kubische Splines auf Zinsen erzeugen unerwünschte Oszillationen und nicht-lokale Sensitivitäten. **Im Core [E]:** Flat-Forward-Extrapolation setzt den *instantanen* Forward am letzten Pillar fort – für Monotone Convex f(t_n) des Hagan-West-Schemas (stetiger Forward über den letzten Pillar, keine Sekante der letzten beiden Knoten), für log-linear/flat-forward den letzten Intervall-Forward. **Bucket-Additivität:** Zero-Buckets je Pillar summieren sich unter lokaler Interpolation (log-linear, flat-forward) zum parallelen DV01; unter nicht-lokaler Interpolation (Monotone Convex mit Positivitätsschranken, Splines) nur näherungsweise (Sample: +0,4 % 10Y-OIS, −0,13 % 30Y) – für Hedge-Zwecke ist das Par-Risiko je Marktquote (`parRisk`/`parRiskPortfolio`) zu verwenden; der Methodiktext weist darauf bei nicht-lokaler Interpolation hin.
...
**CSA ohne Collateral-Kurve im Core [E] (Markt R4-1):** Eine Besicherungswährung wirkt nur dann als CSA-Kurve, wenn der Marktkontext für die Leg-Währung eine kollateralspezifische Diskontkurve kennt (`collateralDiscountCurveId["EUR|USD"]`, z. B. `EUR-ESTR-USDCSA`) oder Leg- und Kollateralwährung identisch sind (Cash-Collateral in eigener Währung → eigene OIS-Kurve). Fehlt die Kurve (EURGBP mit GBP-CSA, EURJPY mit JPY-CSA, GBPUSD/USDJPY mit USD-CSA im Beispielmarkt), diskontiert der Kern auf der Standardkurve der Währung und meldet `COLLATERAL_CURVE_MISSING:` je Währung in `warnings[]`; der Methodiktext schreibt dann „Standard-Diskontkurve – Besicherung in GBP vereinbart, aber … keine Collateral-Kurve" statt „CSA-Kurve", die IFRS-13-Begründung führt den Hinweis (Level 2 mit Hinweis), die Cross-Currency-Basis ist in diesem Fall nicht gepreist.

**MtM-Reset-Cross-Currency-Swaps [E] (Quant R4-1):** Das Nominal des resettenden Legs ist je Periode das Gegen-Nominal × FX-Kurs des Reset-Termins (adjustierter Periodenbeginn). Für Reset-Termine nach dem Bewertungstag ist das der Spot-Date-verankerte Forward-FX-Kurs; für Reset-Termine bis zum Bewertungstag ist der Kurs **fixiert** und wird aus `MarketContext.fxFixings` (`{ pair, date, rate }`, beide Quotierungen des Paars, Snapshot-Feld `fxFixings`, Teil der `marketSnapshotId`) gelesen – die erste Periode ohne Fixing verwendet das kontraktuelle Leg-Nominal. Fehlt das Fixing eines noch bewertungsrelevanten Reset-Termins, wird der heutige Kurs als Näherung verwendet und `MISSING_FX_FIXING:` gemeldet (unter `missingFixingPolicy: "throw"` `PricingError("MISSING_FIXING")`); Fixings vollständig gezahlter Perioden werden nicht verlangt. Damit bleibt die FX-Exposure zwischen zwei Resets im Barwert sichtbar (Probe: 5,4 % des Nominals bei Fixing 1,10 gegen heutigen Kurs 1,1625). Die resettende Leg valuiert frisch nicht exakt par (≈ 0,02 % des Nominals): Zahlungsverzug der Kupons (SOFR Pay-Lag 2) gegen Nominalaustausch am Accrual-Ende.
...
- **Modell/Vol-Typ-Konsistenz im Core [E]:** Wählt ein Trade `model: "Black"`/`"ShiftedBlack"` auf einer Normal-Fläche (oder Bachelier auf einer Lognormal-Fläche), wird die Flächen-Vol je Forward/Strike/Expiry per Preisäquivalenz in die Modell-Quotierung konvertiert (Bachelier-Preis → implizite (shifted) Black-Vol, T11) und `VOL_TYPE_CONVERTED` gewarnt; lognormale Modelle auf nicht-positivem verschobenem Forward/Strike werfen `VOL_MODEL_INCOMPATIBLE`. Eine Normal-Vol von 68 bp wird damit nie mehr als 0,68 % lognormal gelesen; Cap und Swaption liefern unter jedem Modell denselben Preis wie unter Bachelier, das Vega folgt der Modell-Quotierung. Der IFRS-13-Report bleibt Level 2 mit Hinweis.
...
- **Umsetzung in `@deriva/pricing-core` [E]:** Smile-Koordinate ist das unadjustierte Forward-Put-Delta des Strikes (monoton im Strike unter jeder Konvention); Delta-Konventionen Spot, Forward, Premium-Adjusted Spot und Premium-Adjusted Forward (JPY-Crosses, Laufzeiten > 1Y) mit Fixpunkt-Iteration Strike↔Delta (PA-Call auf dem rechten Ast); Butterflies wahlweise als Smile-Strangle (Default) oder **Broker-Strangle** (Reiswich/Wystup 2010: die 25Δ-Strikes werden mit der Ein-Vol σ_ATM + BF gesetzt, die smile-konsistente Strangle-Marge wird so iteriert, dass der Smile diesen Strangle repriziert; 10Δ-Quotes werden als Smile-Strangle gelesen); Interpolation linear oder monoton-kubisch (Fritsch–Carlson) mit flacher Extrapolation jenseits der 10Δ-Pillars. **Beispielflächen** (EURUSD, EURGBP, EURCHF, GBPUSD, USDJPY, EURJPY) deklarieren `deltaConvention: "Spot"` und `atmConvention: "DeltaNeutral"` (Marktkonvention ≤ 1Y, Bloomberg OVML / Reiswich–Wystup-Tabelle; eine Konvention je Fläche, > 1Y wäre Forward-Delta; EUR-Crosses und USDJPY quotieren interbank premium-adjusted, im indikativen Sample unadjustiert). Barrieren (Reiner–Rubinstein) diskontieren die Auszahlung auf das Lieferdatum und stellen sie gegen den Lieferdatums-Forward; Diffusion, Barrier-Drift und Rebate-at-hit-Diskontierung laufen auf dem Expiry-Horizont (Raten bis zum Standard-Lieferdatum der Expiry – für das Standard-Lieferdatum identisch mit der Skalierung r_d·T_del/T, bei nicht-standardisiertem Lag aus den Kurven), der Carry zwischen Spot bei Expiry und Lieferdatums-Forward wird über skalierten Spot und Barrier absorbiert, sodass In + Out = Vanilla für jeden Settlement-Lag exakt gilt und der Rebate-at-hit-Wert nicht vom Lag abhängt; Greeks von Barrieren/Digitals per zentralen finiten Differenzen.

- **Settlement-Konvention FX-Forward/FX-Swap im Core [E] (Quant R4-2):** Ein Leg mit Lieferung **nach** dem Bewertungstag wird diskontiert (fairer Kurs = Forward zum Lieferdatum); ein Leg mit Lieferung **am** Bewertungstag (Value Today, Near-Leg eines O/N-Swaps) ist ein Value-Today-Austausch – beide Beträge gehen undiskontiert zum Heute-Kurs S·DF_Quote(t_s)/DF_Basis(t_s) in den Barwert ein, `nearFairForward`/`fairForward` ist der Heute-Kurs (nicht der Spot), die Swap-Points beziehen sich darauf, und die Warnung `SETTLES_TODAY:` weist die Konvention aus; ein **vor** dem Bewertungstag geliefertes Leg ist ausgeschlossen (Warnung „already delivered"). FX-Swap-Warnungen der Legs werden durchgereicht. Abweichend von QuantLibs Default `includeReferenceDateEvents = false` ist der Value-Today-Austausch damit bewusst Teil des Barwerts – ein Off-Market-Near-Leg kann sonst still verschwinden.
...
- **Umsetzung Sensitivitäten [E]:** Zero-Buckets je Pillar und Par-Risiko je Marktquote (Re-Bootstrap aller abhängigen Kurven; portfolioweit über `parRiskPortfolio`, das jeden gebumpten Kurvensatz einmal baut), Vega je Expiry-Zeile oder Expiry × Tenor (Swaption-Cube) sowie für eingebettete Caps/Floors in Swap-Legs (Feature-Erkennung, nicht Trade-Typ), Theta als Constant-Curve-Roll mit Carry/Roll-Down-Zerlegung. Zero-Buckets sind nur unter lokaler Interpolation exakt additiv zum DV01 (siehe 3.1, Bucket-Additivität); Vega-Buckets summieren sich bis auf Smile-Nichtlinearität (SABR, Delta-Raum) und die Varianz-Interpolation zwischen FX-Expiries (Sample 1,5Y: +2,6 %) zum parallelen Vega. Theta ist der Constant-Curve-Roll auf t+1 plus die in (t, t+1] gezahlten Cashflows – jeder Cashflow genau einmal: FX-Legs, die am Roll-Datum liefern, bleiben als Value-Today-Austausch im gerollten PV und werden nicht zusätzlich als Cash gezählt (`thetaDetail.valueTodayOnRollDate`, Quant-Review R5 N5-1).
- **Reproduzierbarkeit [E]:** `marketSnapshotId` (= `audit.snapshotId` jedes Reports, `X-Market-Snapshot-Id` der API) hasht seit Runde 3 den vollständigen, kanonisch sortierten Snapshot: Bewertungstag, Kurven nach `id` (Basis-Pillars, Turn-of-Year-Sprünge, Interpolation, Extrapolation, Tageszählung), Diskont-/CSA-Mappings, FX-Spots und explizite Spot-Dates, Fixings (sortiert), Swaption-/Caplet-/FX-Flächen inkl. Konventionen, Kreditdaten und Snapshot-Label – reihenfolgeunabhängig; ein Vol-Shift, ein Fixing oder eine geänderte Hazard-Rate ändern die ID. `marketSnapshotId(ctx, { scope: "curves" })` liefert den alten reinen Kurven-Hash.
...
- **CVA/DVA/FVA [Q/E]:** CVA = LGD · Σ EE(t_i)·ΔPD(t_i)·DF(t_i) mit erwarteten Exposures aus Monte Carlo (Hull-White für Zinsen, GBM/FX). Für Vanilla-IRS gibt es semi-analytische Näherungen (Swaption-Replikation). IFRS 13 verlangt Einbezug des Kontrahentenrisikos; CVA/DVA beeinflussen Hedge-Effektivität [Q]. **Umsetzung CDS-Bootstrap [E]:** stückweise konstante Hazards aus Par-Spreads, Prämien-Leg mit **ACT/360**-Accrual (ISDA-Standard, Faktor 365/360 auf der ACT/365F-Hazard-Zeit), Accrual-on-Default und Protection zum Periodenmittelpunkt (ISDA-Standardmodell-Näherung); flache Struktur reproduziert λ = s·(365/360)/(1 − R) (Quant-Review R5 N5-5, Cross-Check gegen QuantLib 1.43 `SpreadCdsHelper`).
...
| T13 | **Golden Master (umgesetzt)** | `packages/pricing-core/test-data/golden/*.json`: Swap und Cash-Swaption (CCP/IRR) auf flacher 3 %-Kurve, €STR-OIS-Compounding, Hull-Caplet (T1), Bachelier-ATM (T3), GK EUR/USD (T2) mit Delivery-Lag, FX-Forward mit Spot-Date, Caplet-Strip, **Sample-Markt-€STR-Bootstrap** (TARGET, Spot T+2, Zahlungsverzug 1, log-linear; Pillars ≤ 1Y geschlossen, 18M…30Y per Bisektion der Par-Gleichung, unabhängige Kalender-/Datumslogik in Python) | Reproduktion durch die Engine auf 1e-6 relativ (Bootstrap-DFs 1e-9/1e-12, Repricing aller Quotes < 1e-9; `src/testing/golden.test.ts`); Referenzen unabhängig per geschlossener Formel bzw. eigener Nachrechnung (`tools/quantlib-golden.py`); **QuantLib-Cross-Check durchgeführt (QuantLib 1.43)**: Flat-Swap, Black-76, Bachelier auf 1e-13, Bootstrap-DF-Verhältnisse auf 1e-12, absolute DFs uniform +1,87·10⁻⁸ aus der 0→Spot-Stub-Konvention (einfach vs. stetig; `test-data/golden/README.md`), `quantlib`-Blöcke eingecheckt und in `golden.test.ts` geprüft | eigene Herleitung [E], QuantLib 1.43 [Q] |
