...
export const marketSnapshotSchema = {
  ...
  properties: {
    ...
    quotes: {
      ...
      maxItems: 200,
      description:
        "API envelope extension (ADR-027 R9/R10, Markt R9-1/R10-1): bootstrap specs of every curve the exporting instance knows quotes for – since round 10 the sample curves at the store's current quotes as well as every curve loaded through `POST /api/market/curves`, and in import mode the imported snapshot's own `quotes` plus curves loaded since (the workstation exports the same block). The curve itself travels in `curves` as before and is imported unchanged; the spec is stored next to it so that `POST /api/risk/par[/portfolio]` bumps the curve's quotes after the import (no `PAR_RISK_INCOMPLETE:` for it; a spec that does not reproduce its curve is reported as `PAR_RISK_INCONSISTENT:` instead of being bumped) and a later `PUT /api/market { valuationDate }` re-bootstraps the curve from them. An imported market bumps only the specs of this block (never the importer's default sample specs): a sample curve without an entry is a curve without quotes. Entries for sample curve ids are kept for the import mode only – `PUT /api/market { discardImport: true }` rebuilds the sample market from its own quotes and drops them (`MARKET_STATE_DROPPED:` when they differ from the sample spec, N10-02). Not part of the snapshot id (`X-Market-Snapshot-Id` is unchanged by the block) but part of the export ETag (order-independent hash). Validated with the market (422 `SNAPSHOT_INVALID`): `curveId` must be a curve of the snapshot, `spec.id`/`spec.currency` must match it, `spec.index` must be registered or listed in `indices` and denominated in the curve's currency. Omitted when the store knows no quotes (an imported snapshot without the block).",
      items: {
        type: "object",
