  ...
  app.get<{ Headers: { "if-none-match"?: string } }>(
    "/api/market/snapshot",
    {
      ...
      schema: {
        ...
        description:
          'The core\'s `deriva.market/1` document (`serializeMarket`). When calendars, indices or swap conventions were registered at runtime (`POST /api/market/calendars|indices|conventions`), the API adds the envelope arrays `calendars` / `indices` / `conventions` (ADR-027) so a re-import restores them – including indices and conventions on composite calendars (`CZ+TARGET`, N9-01), which a fresh process accepts. The API adds `quotes[]` (`{ curveId, spec }` per curve, Markt R9-1) for every curve whose bootstrap spec the store knows – since round 10 (R10-1) the sample curves at their current quotes too, plus every curve loaded through `POST /api/market/curves`; in import mode the imported snapshot\'s own `quotes` and curves loaded since – so par risk works after a re-import in the API and in the workstation and the importer never assumes its own sample quotes. None of it is part of the snapshot id (`X-Market-Snapshot-Id`, `audit.snapshotId` – market data only) but all of it is part of this representation, so the `ETag` is `"<snapshotId>"` while the envelope is empty and `"<snapshotId>-<registerHash>"` otherwise (N8-03; the hash covers register and quotes): a registration or a loaded curve\'s quotes change the ETag, not the header, and a client revalidating with the old ETag gets the fresh body instead of 304.',
        headers: {
          type: "object",
          properties: {
            "if-none-match": {
