...
| `rfr-observation-shift-quantlib` | `OvernightIndexedCoupon(…, Sofr(), Actual360, lookbackDays, 0, applyObservationShift)` on five realised SOFR periods (monthly 31/28 days, Good-Friday start, Good-Friday end, regular, quarterly 92/92) × lookback 1/2/5 × with/without shift (N10-1 / N10-3), plus the projected coupons of a 2Y SOFR swap (lookback 0, lookback 5 ± shift) on `FlatForward(4 %, Actual365Fixed, Continuous)`; `expected` is the ISDA-2021 manual compounding (divisor = observation period under shift, fixing of day d = `obs(inEffect(d))`), bit-identical to QuantLib |
